...
class AdvancedTrajectoryAnalyzer:
    ...
    def _generate_predictions(self):
        ...
        for vehicle_id, positions in self.trajectories.items():
            ...
            velocity = np.mean(velocities[-5:], axis=0) if len(velocities) > 0 else np.zeros(2)
            last_pos = smoothed[-1]
            ...
            # Generate multi-modal predictions using different polynomial degrees
            future_modes = []
            mses = []
            
            for mode_idx in range(self.predictor.num_modes):
                deg = mode_idx + 1
                try:
                    poly_x = np.polyfit(time_steps, smoothed[:, 0], deg=min(deg, len(smoothed)-1))
                    poly_y = np.polyfit(time_steps, smoothed[:, 1], deg=min(deg, len(smoothed)-1))
                except:
                    poly_x = np.polyfit(time_steps, smoothed[:, 0], deg=1)
                    poly_y = np.polyfit(time_steps, smoothed[:, 1], deg=1)
                
                # Compute MSE on history for probability
                pred_hist_x = np.polyval(poly_x, time_steps)
                pred_hist_y = np.polyval(poly_y, time_steps)
                mse = np.mean((pred_hist_x - smoothed[:, 0])**2 + (pred_hist_y - smoothed[:, 1])**2)
                mses.append(mse)
                ...
                for t in range(frames_until_exit):
                    ...
                    # Check if still in frame (with margin)
                    margin = 50
                    if not (-margin <= pred_x <= self.frame_width + margin and -margin <= pred_y <= self.frame_height + margin):
                        break
                    
                    predicted_path.append([pred_x, pred_y])
                
                if len(predicted_path) == 0:
                    predicted_path = [[last_pos[0], last_pos[1]]]
                
                future_modes.append(np.array(predicted_path))
            
            # Calculate mode probabilities based on MSE (lower MSE = higher prob)
            mses = np.array(mses)
            if np.all(mses == mses[0]):
                mode_probs = np.ones_like(mses) / len(mses)
            else:
                logits = -mses
                mode_probs = softmax(logits)
            
            # Debug: verify prediction length for first few vehicles
            if predicted_count < 3:
                print(f"    → Generated {len(future_modes[0])} prediction points using adaptive multi-degree polynomial extrapolation")
            
            self.predictions[vehicle_id] = (np.array(future_modes, dtype=object), mode_probs)
            predicted_count += 1
    ...
    def visualize_predictions(
        ...
        for idx, (vehicle_id, (future_modes, mode_probs)) in enumerate(sorted_vehicles, 1):
            ...
            for mode_idx, (mode_traj, prob) in enumerate(zip(future_modes, mode_probs)):
                ...
                if len(mode_traj) > 0:
                    ...
                    # Draw ENTIRE predicted path with solid lines for smoothness
                    for i in range(1, len(mode_traj)):
                        pt1 = tuple(mode_traj[i-1])
                        pt2 = tuple(mode_traj[i])
                        cv2.line(img, pt1, pt2, color, 2, cv2.LINE_AA)
                
                # Draw endpoint with larger circle if prediction exists
                if len(mode_traj) > 0:
            ...
            info_lines = [
                ...
                f"Prediction Horizon: {len(future_modes[0]) if len(future_modes) > 0 else 0} frames",
                "",
                "Predicted Modes:",
        ...
        for vehicle_idx, (vehicle_id, (future_modes, mode_probs)) in enumerate(self.predictions.items()):
            ...
            if len(future_modes) > 0:
                # Select the mode with highest probability
                best_mode_idx = np.argmax(mode_probs)
                predicted = future_modes[best_mode_idx].astype(int)
                last_pos = tuple(history[-1].astype(int))
